...
OpenAI's API is the de facto standard that most other providers
(DeepSeek, MiMo, Groq, etc.) are compatible with.
"""
...
class OpenAIProvider(LLMProvider):
    ...
    async def _make_request(self, request: LLMRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise ProviderError("Request timed out", provider="openai")
        except httpx.ConnectError as e:
            raise ProviderError(f"Connection failed: {e}", provider="openai")
    ...
    async def _stream_raw(
        ...
        try:
            ...
            raise ProviderError("Stream timed out", provider="openai")
    ...
    def _handle_response(self, response: httpx.Response) -> dict[str, Any]:
        if response.status_code == 200:
            ...
            raise AuthenticationError("Invalid OpenAI API key", 401, "openai")
        elif response.status_code == 429:
            raise RateLimitError("OpenAI rate limit exceeded", 429, "openai")
        elif response.status_code == 400:
            body = response.json()
            msg = body.get("error", {}).get("message", str(body))
            if "context" in msg.lower() or "token" in msg.lower():
                raise ContextOverflowError(msg, 400, "openai")
            raise ProviderError(msg, 400, "openai")
        else:
            raise ProviderError(
    ...
    def _parse_response(self, raw: dict[str, Any]) -> Completion:
        ...
        if "usage" in raw:
            ...
            usage = TokenUsage(
                ...
                cache_read_tokens=u.get("prompt_tokens_details", {}).get(
                    "cached_tokens", 0
                ),
            )
